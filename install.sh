...
# --- IDENTIFICAÇÃO DO SISTEMA E USUÁRIO WEB ---
. /etc/os-release
OS=$ID
VER=$VERSION_ID
...
if [[ "$OS" == "ubuntu" || "$OS" == "debian" ]]; then
    ...
    echo -e "${RED}[ERRO] Sistema operacional não suportado pelo script.${NC}"
    exit 1
fi

# ==========================================
# FUNÇÃO 1: INSTALAÇÃO COMPLETA (NOVA)
# ==========================================
install_all() {
    clear
    ...
    # --- INSTALANDO O PAINEL E O PTEROQ ---
...
EOF
    ...
    # --- INSTALANDO O WINGS E SSL DO NODE ---
...
# ==========================================
# FUNÇÃO 2: ATUALIZAR PAINEL DE FORMA SEGURA
# ==========================================
update_panel() {
    clear
    echo -e "${CYAN}=== ATUALIZANDO PTERODACTYL PANEL ===${NC}"
    if [ ! -d "/var/www/pterodactyl" ]; then
        echo -e "${RED}[ERRO] O painel não parece estar instalado em /var/www/pterodactyl.${NC}"
        return
    fi

    cd /var/www/pterodactyl
    echo -e "${YELLOW}[1/6] Colocando o painel em modo de manutenção...${NC}"
    php artisan down

    echo -e "${YELLOW}[2/6] Baixando a última versão do painel...${NC}"
    curl -L -s -o panel.tar.gz https://github.com/pterodactyl/panel/releases/latest/download/panel.tar.gz

    echo -e "${YELLOW}[3/6] Extraindo e configurando permissões (SEU .ENV ESTÁ SEGURO)...${NC}"
    chmod -R 755 storage/* bootstrap/cache/
    tar -xzvf panel.tar.gz > /dev/null 2>&1
    chmod -R 755 storage/* bootstrap/cache/

    echo -e "${YELLOW}[4/6] Atualizando dependências do Composer...${NC}"
    export COMPOSER_ALLOW_SUPERUSER=1
    composer install --no-dev --optimize-autoloader --no-interaction -q

    echo -e "${YELLOW}[5/6] Limpando caches e atualizando o Banco de Dados...${NC}"
    php artisan view:clear
    php artisan config:clear
    php artisan migrate --seed --force --no-interaction -q

    echo -e "${YELLOW}[6/6] Restaurando permissões e reiniciando serviços...${NC}"
    chown -R $WEB_USER:$WEB_USER /var/www/pterodactyl/*
    systemctl restart pteroq.service

    php artisan up
    echo -e "${GREEN}[SUCESSO] O Painel foi atualizado para a versão mais recente!${NC}"
}

# ==========================================
# FUNÇÃO 3: ATUALIZAR WINGS
# ==========================================
update_wings() {
    clear
    echo -e "${CYAN}=== ATUALIZANDO PTERODACTYL WINGS ===${NC}"
    if [ ! -f "/usr/local/bin/wings" ]; then
        echo -e "${RED}[ERRO] O Wings não parece estar instalado em /usr/local/bin/wings.${NC}"
        return
    fi

    echo -e "${YELLOW}[1/3] Parando o serviço do Wings...${NC}"
    systemctl stop wings

    echo -e "${YELLOW}[2/3] Baixando a versão mais recente do Wings...${NC}"
    curl -L -s -o /usr/local/bin/wings "https://github.com/pterodactyl/wings/releases/latest/download/wings_linux_amd64"
    chmod u+x /usr/local/bin/wings

    echo -e "${YELLOW}[3/3] Iniciando o serviço do Wings...${NC}"
    systemctl start wings

    echo -e "${GREEN}[SUCESSO] O Wings foi atualizado e reiniciado com sucesso!${NC}"
}
...
show_menu() {
    clear
    echo -e "${CYAN}    ___       __           __  ________                __ "
    echo "   / _ | ___ / /________ _/ / / ___/ /___  __ _____   / / "
    echo "  / __ |(_-</ __/ __/ _ \`/ / / /__/ / __ \/ // / _ \ /_/  "
    echo " /_/ |_/___/\__/_/  \_,_/_/  \___/_/\___/\_,_/_//_/ (_)   ${NC}"
    echo -e "${GREEN}======================================================${NC}"
    echo -e "${YELLOW} Central de Gerenciamento - Astral Cloud${NC}"
    echo -e "${GREEN}======================================================${NC}"
    echo -e " 1) ${CYAN}Instalar Novo (Painel + Wings)${NC}"
    echo -e " 2) ${CYAN}Atualizar apenas o Painel${NC}"
    echo -e " 3) ${CYAN}Atualizar apenas o Wings (Node)${NC}"
    echo -e " 4) ${CYAN}Atualizar AMBOS (Painel + Wings)${NC}"
...
# Chama o menu quando o script é executado
show_menu
